...
Given('a product doesn\'t exist', function (dataTable) {
    ...
    return
expect(actions.isElementOnPage(homePage.productInTable(this.product))).to.eve
ntually.be.false;
    });
    When('I add the product', function () {
        ...
        actions.type(addProductPage.productDescription,
    ...
    Then('the product is created', function () {
        return
    expect(actions.waitForElement(viewProductPage.productName(this.product))).to.
eventually.be.true;
    });